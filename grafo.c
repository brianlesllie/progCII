...
int tem_ciclo(grafo_t *grafo){
	no_t *i;
	vertice_t *v;
	for(i=obter_cabeca(grafo->vertices);i!=NULL;i=obtem_proximo(i)){
		v=obter_dado(i);
		vertice_set_pai(v,NULL);
		vertice_set_grupo(v,-1);
	}
	for(i=obter_cabeca(grafo->vertices);i!=NULL;i=obtem_proximo(i)){
		v=obter_dado(i);
		if (e_ciclo(grafo,v)==1)
			return 1;
	}
	return 0;
}
int e_ciclo(grafo_t *grafo,vertice_t* inicial){
	pilha_t *p=cria_pilha();
	no_t *i;
	int num=0;
	vertice_t *u;
	for(i=obter_cabeca(grafo->vertices);i!=NULL;i=obtem_proximo(i)){
		vertice_set_visit(obter_dado(i),0);
		vertice_set_pai(obter_dado(i),NULL);
	}
	push(inicial,p);
	while(!pilha_vazia(p)){
		u=pop(p);
		if ((vertice_get_visit(u))&&(num>=3))
			return 1;
		if (!vertice_get_visit(u)){
			vertice_set_visit(u,1);
			num++;
			for(i=obter_cabeca(vertice_get_arestas(u));i!=NULL;i=obtem_proximo(i)){
                vertice_set_pai(aresta_get_adjacente(obter_dado(i)),u);
				if (vertice_get_pai(u)!=aresta_get_adjacente(obter_dado(i)))
                    push(aresta_get_adjacente(obter_dado(i)),p);
			}
		}
	}
	return 0;
...
void bfs(grafo_t *grafo, vertice_t* inicial)
{
	...
	vertice_set_grupo(inicial,0);
	enqueue(inicial,p);
	while(!fila_vazia(p)){
		u=dequeue(p);
		for(i=obter_cabeca(vertice_get_arestas(u));i!=NULL;i=obtem_proximo(i)){
			v=aresta_get_adjacente(obter_dado(i));
			if(vertice_get_grupo(v)==-1||vertice_get_grupo(v)>(vertice_get_grupo(u)+aresta_get_peso(obter_dado(i)))){
				vertice_set_pai(v,u);
				vertice_set_grupo(v,vertice_get_grupo(u)+aresta_get_peso(obter_dado(i)));
				enqueue(v,p);
			}
		}
	}
